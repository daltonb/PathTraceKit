...
void runCuda() {
    ...
    if(iterations<renderCam->iterations) {
        ...
        // execute the kernel
        cudaRaytraceCore(dptr, cam, iterations, cudamaterials, numberOfMaterials, cudageoms, numberOfGeoms, cudaimage);
...
void initDeviceMemory() {
    ...
    cudaMalloc((void**)&cudageoms, numberOfGeoms*sizeof(geom));
    cudaMemcpy(cudageoms, geoms, numberOfGeoms*sizeof(geom), cudaMemcpyHostToDevice);
...
GLuint initShader(const char *vertexShaderPath, const char *fragmentShaderPath){
    ...
    location = glGetUniformLocation(program, "u_image");
    if (location != -1) {
